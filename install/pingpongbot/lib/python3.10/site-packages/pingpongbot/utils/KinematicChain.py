...
class KinematicChain():
    ...
    def fkin(self, q):
        ...
        for step in self.steps:
            ...
            local_ax = step.nlocal
            joint_num = step.dof
            local_t = step.Tshift
            joint_pos = q[joint_num]
            if step.type is JointType.REVOLUTE:
                # Revolute is a shift followed by a rotation:
                local_pos_t = T_from_Rp(Rotn(local_ax, joint_pos), np.zeros(3))
                T = T @ local_t @ local_pos_t

            elif step.type is JointType.LINEAR:
                # Linear is a shift followed by a translation:
                local_pos_t = T_from_Rp(R_from_RPY(0, 0, 0), local_ax * joint_pos)
                T = T @ local_pos_t @ local_t
            else:
